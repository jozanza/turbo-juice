...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // color = applyEdgeDetection(color, uv);
    ...
    // color.a -= f32(global.tick / 4) % 0.58;
...
fn applyEdgeDetection(color: vec4<f32>, uv: vec2<f32>) -> vec4<f32> {
    ...
    let offset = 0.005;
    let centerColor = textureSample(t_canvas, s_canvas, uv).rgb;
    
    // Check top
    let topColor = textureSample(t_canvas, s_canvas, uv + vec2<f32>(0.0, offset)).rgb;
    if (length(topColor - centerColor) > edgeThreshold) {
        return vec4<f32>(1.0, 1.0, 1.0, color.a);
    }
    
    // Check right
    let rightColor = textureSample(t_canvas, s_canvas, uv + vec2<f32>(offset, 0.0)).rgb;
    if (length(rightColor - centerColor) > edgeThreshold) {
        return vec4<f32>(1.0, 1.0, 1.0, color.a);
    }
    
    // Check bottom
    let bottomColor = textureSample(t_canvas, s_canvas, uv + vec2<f32>(0.0, -offset)).rgb;
    if (length(bottomColor - centerColor) > edgeThreshold) {
        return vec4<f32>(1.0, 1.0, 1.0, color.a);
    }
    
    // Check left
    let leftColor = textureSample(t_canvas, s_canvas, uv + vec2<f32>(-offset, 0.0)).rgb;
    if (length(leftColor - centerColor) > edgeThreshold) {
        return vec4<f32>(1.0, 1.0, 1.0, color.a);
    }
